...
func (tree *tree) find(path string, ctx *Ctx) {
	if tree.canBeStatic[len(path)] {
		route, found := tree.static[path]

		if found {
			ctx.routeCount++
			ctx.matchedRoutes[ctx.routeCount] = route
			// TODO: continue and search all other matched routes
			return
		}
	}
	...
	for {
		...
		if i == uint(len(path)) {
			...
			if i-offset == uint(len(node.prefix)) {
				ctx.routeCount++
				ctx.matchedRoutes[ctx.routeCount] = node.data
				// TODO: continue and search all other matched routes
				return
			}
		...
		if i-offset == uint(len(node.prefix)) {
			...
			if node.wildcard != nil {
				ctx.routeCount++
				ctx.matchedRoutes[ctx.routeCount] = node.wildcard.data
		...
		if path[i] != node.prefix[i-offset] {
			if lastWildcard != nil {
				ctx.routeCount++
				ctx.matchedRoutes[ctx.routeCount] = lastWildcard.data
